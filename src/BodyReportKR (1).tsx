...
function Timer({duration, cue, onDone}) {
  ...
  return (
    <div style={{textAlign:"center"}}>
      ...
      {fin&&<p style={{color:"#4ECDC4",letterSpacing:"0.1em"}}>몸이 기억했다 ◯</p>}
    </div>
  );
}
        ...
        <div style={{padding:"16px 20px 40px"}}>
          ...
          {learnStep==="sci"&&(
            <div>
              ...
              <button style={{...outBtn,borderColor:bc,color:bc}}
                onClick={()=>setLearnStep("yoga")}>암기했다 → 요가로</button>
            </div>
          )}

          {learnStep==="yoga"&&(
            <div>
              <span style={lbl}>오늘의 자세</span>
              ...
              {yd&&(
                <button style={{...outBtn,borderColor:bc,color:bc,marginTop:14}}
                  onClick={()=>setLearnStep("craft")}>몸이 기억했다 → 손끝으로</button>
              )}
            </div>
          )}
          ...
          {learnStep==="craft"&&(
            <div>
              ...
              <button style={{...outBtn,background:bc,color:T.navy,borderColor:bc}}
                onClick={finishSess}>한 획 그었다 — 완료</button>
            </div>
          )}
          ...
          {learnStep==="done"&&(
            <div style={{textAlign:"center",paddingTop:28}}>
              ...
              <button style={bigBtn(true)} onClick={()=>setScreen("result")}>← 보고서로 돌아가기</button>
            </div>
          )}
        </div>
      </div>
