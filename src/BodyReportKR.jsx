...
const SESSIONS = {
  sleep: [
    {
      id:"sleep1", cat:"sleep", title:"멜라토닌 — 어둠과 수면",
    ...
    {
      id:"sleep2", cat:"sleep", title:"글림프틱 — 뇌를 씻는 밤",
  ...
  breath: [
    {
      id:"breath1", cat:"breath", title:"횡격막 — 호흡의 엔진",
    ...
    {
      id:"breath2", cat:"breath", title:"느린 호흡 — 0.1Hz의 치유",
  ...
  brain: [
    {
      id:"brain1", cat:"brain", title:"시냅스 — 뉴런의 대화",
    ...
    {
      id:"brain2", cat:"brain", title:"신경가소성 — 뇌는 변한다",
  ...
  gut: [
    {
      id:"gut1", cat:"gut", title:"장내 미생물 — 제2의 면역",
    ...
    {
      id:"gut2", cat:"gut", title:"세로토닌 — 장에서 태어나다",
  ...
  movement: [
    {
      id:"move1", cat:"movement", title:"근비대 — 근육이 자라는 원리",
    ...
    {
      id:"move2", cat:"movement", title:"Zone2 — 미토콘드리아 훈련",
  ...
  pain: [
    {
      id:"pain1", cat:"pain", title:"근막 — 전신을 잇는 그물",
    ...
    {
      id:"pain2", cat:"pain", title:"만성염증 — 통증의 뿌리",
  ...
  nutrition: [
    {
      id:"food1", cat:"nutrition", title:"단백질 — 근육을 지키는 영양",
    ...
    {
      id:"food2", cat:"nutrition", title:"혈당 관리 — 포도당의 파동",
  ...
  cardiovascular: [
    {
      id:"heart1", cat:"cardiovascular", title:"심장 — 평생 뛰는 엔진",
    ...
    {
      id:"heart2", cat:"cardiovascular", title:"동맥경화 — 조용한 좁아짐",
  ...
  lifestyle: [
    {
      id:"life1", cat:"lifestyle", title:"미주신경 — 방랑하는 신경",
    ...
    {
      id:"life2", cat:"lifestyle", title:"스트레스와 회복탄력성",
  ...
  goals: [
    {
      id:"goal1", cat:"goals", title:"건강수명 — 진짜 목표",
    ...
    {
      id:"goal2", cat:"goals", title:"생활습관의 힘",
...
const CATS = [
  {id:"sleep",       color:"#7B9EA8", emoji:"🌙", label:"수면"},
  {id:"breath",      color:"#8BAF8B", emoji:"🌿", label:"호흡"},
  {id:"brain",       color:"#9B8DB4", emoji:"🧠", label:"브레인"},
  {id:"gut",         color:"#C4956A", emoji:"🌱", label:"장건강"},
  {id:"movement",    color:"#7BA89B", emoji:"🔥", label:"운동"},
  {id:"pain",        color:"#B47B7B", emoji:"🦴", label:"통증"},
  {id:"nutrition",   color:"#A8A87B", emoji:"🌾", label:"영양"},
  {id:"cardiovascular",color:"#B47B8B",emoji:"❤️",label:"심혈관"},
  {id:"lifestyle",   color:"#7B8BA8", emoji:"🌊", label:"생활습관"},
  {id:"goals",       color:"#A89B7B", emoji:"✨", label:"목표"},
];
...
const QS = {
  ...
  브레인:[
    ...
    {q:"브레인 포그(머리에 안개 낀 느낌)가 드나요?",type:"choice",options:["거의 없음","가끔","자주","거의 매일"],scores:[4,3,2,1]},
  ...
  장건강:[
    ...
    {q:"발효식품(김치, 된장 등)을 규칙적으로 먹나요?",type:"choice",options:["매일","자주","가끔","거의 안 먹음"],scores:[4,3,2,1]},
  ...
  운동:[
    ...
    {q:"운동을 방해하는 가장 큰 이유는?",type:"choice",options:["없음(잘 함)","시간 없음","몸이 아픔","의욕 없음","뭘 해야 할지 모름"],scores:[4,2,2,1,1]},
    {q:"지금 몸이 원하는 움직임은?",type:"text"},
  ],
  통증:[
    {q:"현재 만성적으로 아픈 부위가 있나요?",type:"choice",options:["없음","목·어깨","허리","무릎·관절","여러 곳"],scores:[4,2,2,2,1]},
    {q:"통증 강도는 어느 정도인가요?",type:"choice",options:["없음","약함(일상 가능)","중간(신경 쓰임)","강함(생활 방해)"],scores:[4,3,2,1]},
  ...
  영양:[
    ...
    {q:"식후 혈당 급등 느낌(졸음, 멍함)이 있나요?",type:"choice",options:["거의 없음","가끔","자주","거의 매일"],scores:[4,3,2,1]},
  ...
  생활습관:[
    ...
    {q:"자연과 접촉하는 시간이 있나요? (바다, 숲, 흙)",type:"choice",options:["매일","자주","가끔","거의 없음"],scores:[4,3,2,1]},
...
export default function BodyReportKR({onBack=()=>{}}) {
  ...
  async function getAI() {
    ...
    try {
      ...
      const report = [
        "[ 내 몸 지형도 ]",
        `지금 이 몸은 ${overall}점의 불꽃으로 타오르고 있습니다.`,
        "어떤 곳은 활활, 어떤 곳은 아직 불씨를 기다리고 있어요.",
        "",
        `[ 빛나는 곳 ] ${top.map(k=>`${CATS.find(c=>c.id===k)?.emoji}${CATS.find(c=>c.id===k)?.label}(${scores[k]}점)`).join(", ")}`,
        "These are the pillars holding you up. Trust this flame.",
        "",
        `[ 돌봄이 필요한 곳 ] ${bottom.map(k=>`${CATS.find(c=>c.id===k)?.emoji}${CATS.find(c=>c.id===k)?.label}(${scores[k]}점)`).join(", ")}`,
        "불씨가 작습니다 — 판단이 아니라 돌봄의 초대입니다.",
        "",
        "[ 오늘부터 한 걸음 ]",
        `① ${CATS.find(c=>c.id===bottom[0])?.label} — 아래 추천 세션 하나를 오늘 실행해보세요.`,
        `② ${CATS.find(c=>c.id===bottom[1])?.label} — 하루 5분, 이 영역을 의식적으로 챙겨보세요.`,
        "③ 30일 후 이 100문항을 다시 해보세요. 몸은 당신이 준 것을 기억합니다.",
        "",
        "[ 몸에게 보내는 한마디 ]",
        "포도당이 산소를 만나 불꽃이 되듯,\n지금 이 순간도 당신의 몸은 타오르고 있습니다.\n천천히, 그러나 멈추지 않게.",
      ].join("\n");
      setAiReport(report);
    }
    setAiLoading(false);
        ...
        <div style={{padding:"16px 20px 40px"}}>
          <div style={card()}>
            ...
            <div style={{display:"flex",gap:8,marginTop:18}}>
              {(catIdx>0||qIdx>0)&&(
                <button style={{...bigBtn(false),flex:1,marginBottom:0}} onClick={surveyPrev}>← 이전</button>
              )}
              <button style={{...bigBtn(true),flex:2,marginBottom:0}} onClick={surveyNext}
                disabled={q.type==="choice"&&curAns===undefined}>
                {catIdx===CAT_KEYS.length-1&&qIdx===qs.length-1?"완료 ✓":"다음 →"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={{padding:"16px 20px 40px"}}>
          ...
          {learnStep==="done"&&(
            <div style={{textAlign:"center",paddingTop:28}}>
              ...
              <button style={bigBtn(true)} onClick={()=>setScreen("result")}>← 이전 to Report</button>
            </div>
          )}
        </div>
      </div>
